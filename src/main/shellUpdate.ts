...
function officialReleaseBase(raw: string): URL | null {
    try {
        const url = new URL(raw);
        const prefix = `/${DOCKVIEW_RELEASE_REPOSITORY}/releases/download/`;
        if (
            url.protocol !== "https:" ||
            ...
            !/^v\d+(?:\.\d+)+$/.test(url.pathname.slice(prefix.length).replace(/\/$/, ""))
        )
            return null;
...
function installerMatchesMethod(method: InstallMethod, url: URL): boolean {
    const path = url.pathname.toLowerCase();
    if (method === "win-nsis") return path.endsWith(".exe");
    if (method === "appimage") return path.endsWith(".appimage");
    if (method === "deb") return path.endsWith(".deb");
    if (method === "rpm") return path.endsWith(".rpm");
    return false;
}
 ...
 */
export async function applyShellUpdate(shellInput: unknown, baseUrl: string): Promise<ShellApplyResult> {
    const releaseBase = officialReleaseBase(baseUrl);
    if (!releaseBase || !shellInput || typeof shellInput !== "object") {
        return { ok: false, error: "Update source is not an official DockView release." };
    }
    const shell = shellInput as ShellManifest;
    const method = await detectInstallMethod();
    const installer = pickInstaller(shell, method, process.arch);
    const tag = releaseBase.pathname.split("/").filter(Boolean).pop()!;
    const manualReleaseUrl = `https://github.com/${DOCKVIEW_RELEASE_REPOSITORY}/releases/tag/${tag}`;

    // No installer for us in this release, or we can't drive this method → manual.
    if (method === "unknown") {
        return { ok: false, manual: true, url: manualReleaseUrl };
    }
    if (!installer || !installer.url || !installer.sha256) {
        return { ok: false, manual: true, url: manualReleaseUrl };
    }

    const url = resolveUrl(installer.url, releaseBase.href);
    let installerUrl: URL;
    try {
        installerUrl = new URL(url);
    } catch {
        return { ok: false, error: "The installer URL is invalid." };
    }
    if (!installerUrl.href.startsWith(releaseBase.href) || !installerMatchesMethod(method, installerUrl)) {
        return { ok: false, error: "The installer is outside the approved DockView release." };
    }
    const dir = stagingDir();
    const dest = join(dir, basename(installerUrl.pathname) || "installer.bin");
