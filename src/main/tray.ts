...
import { createDockViewProfileSwitchItems, getDockViewTrayTooltip } from "dockview/main/trayProfiles";
...
export async function initTray(win: BrowserWindow, setIsQuitting: (val: boolean) => void) {
    ...
    const { activeProfile, switchItems } = createDockViewProfileSwitchItems(setIsQuitting);
    ...
    tray.setToolTip(getDockViewTrayTooltip(activeProfile));
