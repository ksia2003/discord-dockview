...
import { getDockViewUserAgent, resolveDockViewDataPaths } from "dockview/main/earlyBootstrap";
...
// Resolve and apply the profile-specific userData path synchronously during module
// evaluation, before index.ts asks Electron for the single-instance lock.
const dockViewDataPaths = resolveDockViewDataPaths({
    getDefaultUserData: () => app.getPath("userData"),
    portableDataDir: PORTABLE ? join(vesktopDir, "Data") : undefined
});

export const DATA_DIR = dockViewDataPaths.dataDir;
export const BASE_DATA_DIR = dockViewDataPaths.baseDataDir;

if (dockViewDataPaths.shouldSetUserData) {
...
export const USER_AGENT = getDockViewUserAgent(app.getVersion());
