 ...
 *
 * The context tab is a SINGLETON: always present, always index 0, not closable, not
 * draggable, no ✕. Its content follows the current channel — a guild channel shows the
 * member list, a DM shows the profile sidebar (the Discord components acquired by
 * host/slotComponents.ts, rendered in our own React root). It is NOT a DockWindow and
 * does NOT live in channelTabs (it isn't channel-owned file content), so the whole
 * window/tab/dedup/reorder machinery in window.ts is untouched by it.
 *
