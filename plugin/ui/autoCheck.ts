...
import { getNative, getShellVersion, isNewer, OWNER, REPO, shellIsNewer } from "./updateShared";
...
async function runCheck(): Promise<void> {
    try {
        ...
        if (latestVer) {
            const installed = await native.readInstalledVersion().catch(() => null);
        ...
        updateFlagged = true;
        showUpdateNotice(pluginUpdate && latestVer ? latestVer : requiredShell!);
    } catch {
