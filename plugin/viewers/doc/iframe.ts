...
// ---------------------------------------------------------------------------
// THEME INJECTION (option A). A sandboxed srcdoc iframe has its OWN document — it
// can neither reach style.css nor read Discord's --* theme vars. So the doc styles
// below inlined a fixed dark-theme palette (the default-theme hexes). To make the
// iframe FOLLOW the active Discord theme (default AND custom) and stay CONSISTENT
// with the dock body around it, we read the live theme values IN THE RENDERER (which
// CAN read them) at srcdoc-build time and inject them as the iframe's OWN custom
// properties. Every doc style then references `var(--dv-*, <old default hex>)`, so:
//   - where Discord defines the token → the iframe paints the live theme value
//     (matching the dock body, and following custom themes);
//   - where a build leaves the token empty → the CSS falls back to the exact old
//     default-theme hex, so the default appearance is UNCHANGED.
//
// The values Discord exposes are self-contained (e.g. `color-mix(in oklab, hsl(...)
// 100%, #000 0%)` — no nested var()), so the raw string paints correctly inside the
// null-origin sandbox with no host access. We map each --dv-* alias to the semantic
// Discord token whose LIVE value matches the dock's own surfaces (the dock base uses
// the --background-base-* / surface scale, NOT the legacy --background-primary family,
// which is empty on current builds), so the iframe reads as the same surface as the
// dock rather than a divergent hardcoded shade.
//
// --dv alias         Discord token (live default ≈)      old hardcoded hex it replaces
// --dv-bg            --background-base-lower  (#1a1a1e)   page bg (#1e1f22 → now matches dock body)
// --dv-surface       --background-surface-highest (#2c2d32) panel surface (#2b2d31)
// --dv-surface-alt   --background-surface-high (#242429)  alt surface (#232428 / #26282c)
// --dv-border        --background-modifier-accent (unset) hairlines (#3f4147)
// --dv-border-strong --border-subtle          (#212124)   pre/table/card border (#1e1f22)
// --dv-text          --text-default           (#efeff1)   body text (#dbdee1)
// --dv-text-strong   --text-strong            (#fbfbfb)   headings (#f2f3f5)
// --dv-text-muted    --text-muted             (#96979e)   muted (#949ba4)
// --dv-link          --text-link              (#4d96ee)   links (#00a8fc)
// --dv-scrollbar     --scrollbar-thin-thumb   (#5f606a)   scrollbar thumb (#5f606a, exact)
const THEME_VAR_MAP: [alias: string, discordVar: string][] = [
    ["--dv-bg", "--background-base-lower"],
    ["--dv-surface", "--background-surface-highest"],
    ["--dv-surface-alt", "--background-surface-high"],
    ["--dv-border", "--background-modifier-accent"],
    ["--dv-border-strong", "--border-subtle"],
    ["--dv-text", "--text-default"],
    ["--dv-text-strong", "--text-strong"],
    ["--dv-text-muted", "--text-muted"],
    ["--dv-link", "--text-link"]
];
// The scrollbar thumb is mapped separately because its default hex is used directly
// in the scrollbar block below (kept as a --dv alias for one source of truth).
THEME_VAR_MAP.push(["--dv-scrollbar", "--scrollbar-thin-thumb"]);

/** Build the `:root { --dv-*: … }` block injected into every doc iframe. Reads the
 *  live Discord theme values ONCE (called once per srcdoc build), so all doc viewers
 *  reuse the same read. Only emits an alias when Discord actually defines the token,
 *  so an unset token cleanly falls through to the old default hex in the CSS. The raw
 *  value strings are self-contained (no nested var()), so they paint inside the
 *  null-origin sandbox. Returns "" if the DOM/getComputedStyle isn't available. */
export function themeVarBlock(): string {
    try {
        const cs = getComputedStyle(document.documentElement);
        const decls: string[] = [];
        for (const [alias, token] of THEME_VAR_MAP) {
            const v = cs.getPropertyValue(token).trim();
            if (v) decls.push(`${alias}:${v};`);
        }
...
html, body { margin: 0; padding: 0; background: var(--dv-bg, #1e1f22); }
/* Scrollbars: this is a sandboxed srcdoc iframe with its own document, so it can
   neither reach our style.css nor see Discord's --scrollbar-* theme vars DIRECTLY.
   Instead the renderer injects the live thumb colour as --dv-scrollbar (see
   themeVarBlock), so the markdown body's vertical bar and any wide code-block / table
   horizontal bars follow the ACTIVE theme's thin scrollbar — the SAME
   --scrollbar-thin-thumb the dock body/code/CSV scrollers use in style.css, so every
   scroller in the dock (frame and non-frame) is identical. #5f606a is what that token
   resolves to on the default dark theme (the fallback here, so the default look is
   unchanged when the var isn't injected). We do NOT copy Discord's 2px transparent
   padding-box border: that inset shrinks the visible thumb to ~half the bar, and the
   owner wants the thumb to FILL the full bar width like the old default scroller did.
   So: an 8px track, a rounded thumb painted edge-to-edge (no border, no padding-box
   clip), transparent track. Same fade-on-hover as Discord's .fade scrollers. Keep this
   in sync with the .dockview-body/.dockview-cm .cm-scroller rules in style.css. */
...
pre::-webkit-scrollbar-thumb, table::-webkit-scrollbar-thumb {
  background-color: var(--dv-scrollbar, #5f606a);
...
.md {
  ...
  color: var(--dv-text, #dbdee1);
  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji";
  font-size: 15px;
  line-height: 1.6;
  word-wrap: break-word;
}
.md h1, .md h2, .md h3, .md h4, .md h5, .md h6 { color: var(--dv-text-strong, #f2f3f5); font-weight: 600; line-height: 1.3; margin: 24px 0 12px; }
.md h1 { font-size: 1.9em; border-bottom: 1px solid var(--dv-border, #3f4147); padding-bottom: .3em; }
.md h2 { font-size: 1.5em; border-bottom: 1px solid var(--dv-border, #3f4147); padding-bottom: .3em; }
.md h3 { font-size: 1.25em; }
.md h4 { font-size: 1.05em; }
.md p { margin: 0 0 14px; }
.md a { color: var(--dv-link, #00a8fc); text-decoration: none; }
.md a:hover { text-decoration: underline; }
.md ul, .md ol { margin: 0 0 14px; padding-left: 2em; }
.md li { margin: 4px 0; }
.md li > p { margin: 0; }
.md blockquote { margin: 0 0 14px; padding: 0 1em; color: var(--dv-text-muted, #b5bac1); border-left: 4px solid #4e5058; }
.md hr { height: 1px; border: 0; background: var(--dv-border, #3f4147); margin: 24px 0; }
.md img { max-width: 100%; }
.md code { font-family: Consolas, "Andale Mono WT", "Andale Mono", monospace; font-size: 85%; background: var(--dv-surface, #2b2d31); padding: .2em .4em; border-radius: 4px; }
.md pre { background: var(--dv-surface, #2b2d31); padding: 14px 16px; border-radius: 6px; overflow: auto; margin: 0 0 14px; border: 1px solid var(--dv-border-strong, #1e1f22); }
...
.md table th, .md table td { border: 1px solid var(--dv-border, #3f4147); padding: 6px 13px; }
.md table th { background: var(--dv-surface, #2b2d31); font-weight: 600; }
.md table tr:nth-child(2n) { background: var(--dv-surface-alt, #26282c); }
...
.dv-docx-note { color: var(--dv-text-muted, #949ba4); font-size: 12px; margin: 0 0 14px; padding-bottom: 10px; border-bottom: 1px solid var(--dv-border, #3f4147); }
...
.odp-slide { background: var(--dv-surface, #2b2d31); border: 1px solid var(--dv-border, #3f4147); border-radius: 8px; padding: 16px 18px; }
.odp-slide-num { color: var(--dv-text-muted, #949ba4); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; margin: 0 0 10px; }
.odp-box { margin: 0 0 10px; }
.odp-box:last-child { margin-bottom: 0; }
.odp-box > :first-child { margin-top: 0; }
.odp-box > :last-child { margin-bottom: 0; }
.odp-empty { color: var(--dv-text-muted, #80848e); font-style: italic; margin: 0; }
...
.dv-mermaid-error { color: #f85149; white-space: pre-wrap; word-break: break-word; background: var(--dv-surface, #2b2d31); padding: 12px 14px; border-radius: 6px; border: 1px solid var(--dv-border-strong, #1e1f22); }
...
.dv-nb-out pre { margin: 0; background: var(--dv-surface-alt, #232428); border: 1px solid var(--dv-border-strong, #1e1f22); padding: 10px 12px; border-radius: 6px; overflow: auto; white-space: pre-wrap; word-break: break-word; font-family: Consolas, "Andale Mono", monospace; font-size: 12.5px; line-height: 1.45; color: #c7ccd1; }
.dv-nb-out-html { background: var(--dv-surface-alt, #232428); border: 1px solid var(--dv-border-strong, #1e1f22); padding: 10px 12px; border-radius: 6px; overflow: auto; }
.dv-nb-out-html table { border-collapse: collapse; }
.dv-nb-out-html th, .dv-nb-out-html td { border: 1px solid var(--dv-border, #3f4147); padding: 4px 8px; }
...
.dv-nb-sep { height: 1px; border: 0; background: var(--dv-surface, #2b2d31); margin: 0 0 14px; }
...
.dv-eml-head { margin: 0 0 18px; padding: 0 0 14px; border-bottom: 1px solid var(--dv-border, #3f4147); }
.dv-eml-subject { color: var(--dv-text-strong, #f2f3f5); font-size: 1.35em; font-weight: 600; line-height: 1.3; margin: 0 0 10px; word-break: break-word; }
.dv-eml-row { display: flex; gap: 8px; font-size: 13px; line-height: 1.5; margin: 2px 0; }
.dv-eml-label { flex: 0 0 64px; color: var(--dv-text-muted, #949ba4); text-align: right; user-select: none; }
.dv-eml-val { flex: 1 1 auto; min-width: 0; color: var(--dv-text, #dbdee1); word-break: break-word; }
.dv-eml-val .dv-eml-addr { color: var(--dv-text, #dbdee1); }
.dv-eml-val .dv-eml-email { color: var(--dv-text-muted, #949ba4); }
.dv-eml-att { margin: 16px 0 0; padding: 12px 0 0; border-top: 1px solid var(--dv-border, #3f4147); }
.dv-eml-att-title { color: var(--dv-text-muted, #949ba4); font-size: 12px; text-transform: uppercase; letter-spacing: .03em; margin: 0 0 8px; }
.dv-eml-att-list { list-style: none; margin: 0; padding: 0; }
.dv-eml-att-item { display: flex; align-items: center; gap: 8px; padding: 6px 10px; margin: 4px 0; background: var(--dv-surface, #2b2d31); border: 1px solid var(--dv-border-strong, #1e1f22); border-radius: 6px; font-size: 13px; color: var(--dv-text, #dbdee1); }
.dv-eml-att-item .dv-eml-att-icon { flex: 0 0 auto; color: var(--dv-text-muted, #949ba4); }
.dv-eml-att-item .dv-eml-att-name { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.dv-eml-att-item .dv-eml-att-size { flex: 0 0 auto; color: var(--dv-text-muted, #949ba4); font-size: 12px; }
.dv-eml-body { color: var(--dv-text, #dbdee1); line-height: 1.6; word-wrap: break-word; }
.dv-eml-body img { max-width: 100%; }
.dv-eml-body a { color: var(--dv-link, #00a8fc); text-decoration: none; }
.dv-eml-body a:hover { text-decoration: underline; }
.dv-eml-body blockquote { margin: 0 0 14px; padding: 0 1em; color: var(--dv-text-muted, #b5bac1); border-left: 4px solid #4e5058; }
...
.dv-eml-blocked { display: inline-block; padding: 1px 8px; margin: 2px; background: var(--dv-surface, #2b2d31); border: 1px dashed #4e5058; border-radius: 4px; color: var(--dv-text-muted, #949ba4); font-size: 12px; }
.dv-eml-empty { color: var(--dv-text-muted, #949ba4); font-style: italic; }
</style>`;
...
.md .md-math-fallback { color: #f85149; background: var(--dv-surface, #2b2d31); }
</style>`;
...
export function wrapMarkdownDoc(bodyHtml: string, hasMath: boolean): string {
    ...
    // themeVarBlock first so the injected --dv-* custom properties are defined before
    // the doc styles reference them; the styles fall back to the default hex where a
    // token is unset, so a missing block leaves the default appearance unchanged.
    return `<!doctype html><html><head><meta charset="utf-8"><base target="_blank">${themeVarBlock()}${MD_STYLE}${mathStyle}</head><body><article class="md">${bodyHtml}</article>${MD_LINK_SCRIPT}</body></html>`;
}
...
.dv-fm { margin: 0 0 20px; padding: 4px 0 0; border-bottom: 1px solid var(--dv-border, #3f4147); }
.dv-fm-row { display: flex; gap: 12px; padding: 5px 0; font-size: 13.5px; line-height: 1.5; }
.dv-fm-key { flex: 0 0 30%; max-width: 180px; color: var(--dv-text-muted, #949ba4); font-weight: 600; word-break: break-word; }
.dv-fm-val { flex: 1 1 auto; min-width: 0; color: var(--dv-text, #dbdee1); word-break: break-word; white-space: pre-wrap; }
.dv-fm-raw .dv-fm-val { color: var(--dv-text-muted, #b5bac1); font-family: Consolas, "Andale Mono", monospace; font-size: 12.5px; }
...
.dv-copy-btn {
  ...
  background: var(--dv-surface-alt, #383a40); color: var(--dv-text-muted, #b5bac1);
  border: 1px solid var(--dv-border-strong, #1e1f22); border-radius: 4px;
...
.dv-copy-btn:hover { background: #4e5058; color: var(--dv-text, #dbdee1); }
...
.dv-toc {
  ...
  background: var(--dv-surface-alt, #232428); border-left: 1px solid var(--dv-border, #3f4147);
...
.dv-toc-title { color: var(--dv-text-muted, #949ba4); font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .04em; margin: 0 0 10px; }
.dv-toc a {
  display: block; padding: 4px 8px; margin: 1px 0; border-radius: 4px;
  color: var(--dv-text-muted, #b5bac1); font-size: 13px; line-height: 1.4; text-decoration: none;
  word-break: break-word; cursor: pointer;
}
.dv-toc a:hover { background: #35373c; color: var(--dv-text, #dbdee1); }
...
export function wrapMarkdownDocFull(bodyHtml: string, hasMath: boolean, frontmatterHtml: string, toc: { id: string; text: string; level: number }[]): string {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><base target="_blank">${themeVarBlock()}${MD_STYLE}${MD_ENHANCE_STYLE}${mathStyle}</head><body>`
