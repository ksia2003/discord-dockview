 ...
 *
 * This deliberately mirrors engine/lazyLib.ts's loadChunk: same VesktopNative dir
 * lookup, same VencordNative.pluginHelpers.DockView.readChunk call, same eval-via-
...
/** Locate the Vencord files dir the chunk ships into (same lookup lazyLib uses). */
function vencordDir(): string | null {
    try {
        const d = (window as any).VesktopNative?.fileManager?.getVencordDir?.();
...
export function loadSampleChunk(): Promise<SampleMap> {
    ...
    const start = (async (): Promise<SampleMap> => {
        ...
        if (!dir) throw new Error("Cannot locate the Vencord files dir to load the samples.");
        ...
        const src: string | null = await native.readChunk(dir, SAMPLE_CHUNK_FILE);
