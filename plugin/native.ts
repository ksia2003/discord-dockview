...
const REQUIRED_UPDATE_FILES = new Set([
    ...CORE_INSTALL_FILES,
...
let approvedUpdate: { manifestJson: string; baseUrl: string; expiresAt: number } | null = null;

function isAllowedInstallFile(name: string): boolean {
    return CORE_INSTALL_FILES.has(name) || /^chunk-[A-Za-z0-9._-]+\.js$/.test(name);
}
 ...
 */
export async function readInstalledVersion(_: IpcMainInvokeEvent): Promise<string | null> {
    try {
        const raw = await readFile(join(INSTALL_DIR, VERSION_FILE), "utf-8");
        return raw.trim();
    } catch {
        return null;
    }
 ...
 * and joined onto INSTALL_DIR, so it cannot escape the install dir via "../" or an
 ...
 */
export async function readChunk(_: IpcMainInvokeEvent, name: string): Promise<string | null> {
    // Only `chunk-<safe>.js`, no path separators — cannot traverse out of INSTALL_DIR.
    if (typeof name !== "string" || !/^chunk-[A-Za-z0-9._-]+\.js$/.test(name)) return null;
    try {
        return await readFile(join(INSTALL_DIR, name), "utf-8");
    } catch {
        return null;
    }
}
...
export async function applyUpdate(
    ...
    const invalidName = names.find(name => !isAllowedInstallFile(name));
    if (invalidName) {
    ...
    if (missingName || names.length !== REQUIRED_UPDATE_FILES.size) {
        return {
            ...
            needsRelaunch: false,
            error: missingName
                ? `Manifest is incomplete; missing "${missingName}"`
                : "Manifest file set does not match the DockView runtime"
        };
    }
    const commitOrder = [
    ...
    const tmpPathOf = (name: string) => join(INSTALL_DIR, name + TMP_SUFFIX);
    ...
    try {
        // Ensure the active install directory exists.
...
export async function discoverManifest(
    ...
    approvedUpdate = null;
    if (owner !== RELEASE_OWNER || repo !== RELEASE_REPO) {
        return { ok: false, code: "noRelease" };
    }
